...
class IZAKernel {
  ...
  constructor() {
    ...
    this.directus = createDirectus('http://localhost:8055').with(rest()); // Assuming Directus runs on 8055
  ...
  async routePrompt(prompt: string) {
    ...
    if (lower.includes('memory')) {
      return { tool: 'memoryWrite', payload: { prompt } };
    }
    if (lower.includes('deploy')) {
      return { tool: 'deploy', payload: { prompt } };
    }
    return { tool: 'backtest', payload: { prompt } };
  }
